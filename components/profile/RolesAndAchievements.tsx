    ...
    <div className="mb-12 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-4">
        {roles.map((role) => (
          <span
            key={role.name}
            className={`${role.color} text-navy-900 font-semibold px-3 py-1 rounded-full text-sm animate-fade-in tag-neon`}
          >
            {role.name}
          </span>
        ))}
        {achievements.map((ach) => (
          <span
            key={ach.name}
            className="bg-cyan-400 text-navy-900 font-semibold px-3 py-1 rounded-full text-sm animate-fade-in tag-neon"
          >
            {ach.icon} {ach.name}
          </span>
        ))}
